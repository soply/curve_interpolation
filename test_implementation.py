...
def error_test(original_curve, t0 , t1, Np,numberOfLevels,lam):
	...
	for i in range(numberOfLevels):
		projections = (ymin[:,i] + np.outer(tangents_per_levelset[:,i], (tangents_per_levelset[:,i])).dot((x[:,labels == i+1].T - ymin[:,i]).T).T).T
		err += np.sum(np.square(np.linalg.norm(projections - curve[:,labels == i+1], axis = 1)))
		# c = 0
...
if __name__ == '__main__':
    ...
    dummy_for_shape = np.zeros((len(lam), len(levels))
    err = np.memmap(os.path.join(tmp_folder, 'err'), dtype='float64',
	...
	err = Parallel(n_jobs=1)(delayed(error_test)
