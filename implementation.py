...
def reconstructCurve(t0, t1, points, diffpoints, dom, N,numberOfLevels, lam=1):
	...
	def getNumberOfPointsInLevel(i):
		return len(np.where(labels == i+1)[0])
		# c = 0
	...
	# timenew = time.time()
	# print 'Creating A: ', timenew - start
	# start = timenew
	...
	# timenew = time.time()
	# print 'Creating b: ', timenew - start
	# start = timenew

	ymin = spl.spsolve(A,b)

	# timenew = time.time()
	# print 'Solving lin sts: ', timenew - start
	# start = timenew
